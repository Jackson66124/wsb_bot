...
import { ALPACA_DISCONNECT_URL } from "../config";
...
function Connected() {
  ...
  const handleClick = (e, linkType) => {
    ...
    if (linkType === "disconnect") {
      window.open(ALPACA_DISCONNECT_URL, "_blank", "noopener,noreferrer");
      ...
      <div>
        <div className="top-stock">
          ...
          <p className="top-stock-description">
            ACCOUNT CONNECTED!
            <br />
            Your Alpaca account has been successfully connected to our sentiment analysis bot.
            The bot will now begin placing orders at 12pm every day that the stock market is open.
          </p>
        </div>
      </div>
      <div className="disconnect-button-container">
        <button className="disconnect-button" onClick={(e) => handleClick(e, "disconnect")}>
          Disconnect Account
        </button>
      </div>
      <StockDatepicker />
      <div className="reconnect-account">
        Bot no longer making orders on your Alpaca account?
        <button className="connect-account-button" onClick={(e) => handleClick(e, "connect")}>
          Reconnect it here!
        </button>
      </div>
    </div>
